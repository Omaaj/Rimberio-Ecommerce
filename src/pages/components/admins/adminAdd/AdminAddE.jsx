...
export default function AdminAddE() {
    const {id} = useParams();
    ...
    const handleImageChange = (e) => {
      ...
      // console.log(file);
      const storageRef = ref(storage, `rimberiossElectro/${Date.now()}${file.name}`);
    ...
    const addProducts = (e) => {
      e.preventDefault();
      try {
        const docRef = addDoc(collection(db, "electro-products"), {
        ...
        navigate("/admin/all-electronics-products")
      } catch (error) {
        toast.error(error.message)
      }
    }
    ...
    const editProducts = (e) => {
      ...
      try {
        setDoc(doc(db, "electro-products", id), {
          title: products.title,
          cat : products.cat,
          brand : products.brand,
          oldprice : Number(products.oldprice),
