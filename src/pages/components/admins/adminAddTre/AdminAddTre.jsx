...
export default function AdminAddTre() {
  const {id} = useParams();
    ...
    const handleInputChange = (e) => {
      ...
      const handleImageChange = (e) => {
        ...
        // console.log(file);
        const storageRef = ref(storage, `rimberioss2/${Date.now()}${file.name}`);
    ...
    const addProducts = (e) => {
        e.preventDefault();
        try {
          const docRef = addDoc(collection(db, "trending-products"), {
          ...
          navigate("/admin/all-trending-products")
        } catch (error) {
          toast.error(error.message)
        }
    }
    ...
    const editProducts = (e) => {
        ...
        try {
          setDoc(doc(db, "trending-products", id), {
            productId: products.productId,
            title: products.title,
            oldPrice : Number(products.oldPrice),
            price : Number(products.price),
      ...
      return (
        <div className='admin_productss'>
          <h1>{detectForm(id, "Add Trending Product", "Edit Product")}</h1>
          <div className="cards">
